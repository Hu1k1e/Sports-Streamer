...
- **[2026-07-14]**: Fixed 'Stream not playing / buffering loops' caused by broken high-priority sources (e.g., admin streams). (1) Removed check_better_source_available which was intentionally dropping working fallback streams (like golf) to retry broken high-priority streams, causing infinite buffering loops. (2) Changed background stream Health Check to use GET with a Range: bytes=0-1024 header instead of HEAD, preventing anti-piracy CDNs from returning 403 Forbidden and falsely evicting healthy streams. (3) Drastically reduced Playwright scrape timeouts (from ~25s to ~8s per embed URL) by breaking the element-click loop early. This ensures that if the top 5 sources are dead, the scraper quickly falls back to a working source before Jellyfin times out.

- **[2026-07-14]**: Added _dead_embed_urls cache in scraper.py with a 10-minute TTL. Streams that fail to extract an m3u8 playlist are temporarily blacklisted. This prevents the scraper from re-testing known-broken high-priority streams on every stream restart, while still allowing them to be retried after 10 minutes in case they come back online.
...
- **[2026-07-15]**: Implemented Seamless HLS Mid-Stream Failover & Background Hot-Standby Architecture. (1) Upgraded get_stream_urls to fetch up to 3 working streams simultaneously. (2) Re-architected stream_cache to hold a list of redundant streams. (3) Created  ctive_stream_keeper_task that continuously sends heartbeat GET requests with Range: bytes=0-1024 to backup CDNs, keeping their session tokens alive without invoking Playwright. (4) If a backup CDN token expires, a background _replenish_backups Playwright task is spawned silently to find a new backup. (5) Intercepted sub-playlist 502/404 failures in handle_proxy_m3u8 to instantly swap to the hot backup's sub-playlist and inject #EXT-X-DISCONTINUITY, allowing the video decoder to natively and seamlessly switch sources mid-stream without Jellyfin dropping the connection. (6) Initial stream playback retains zero-delay by instantly returning the first working stream from the first batch and deferring backup discovery to the background keeper.
- **[2026-07-15]**: Fixed infinite failover bug by resolving URL persistence during mid-stream swaps. Added native integration for 10 exclusive 24/7 streams from LiveXTV (including Fox 4K) via a dedicated Playwright scraper module (`scraper_livextv.py`), automatically prepended to the top of the M3U and EPG guides under a dedicated "24/7 Channels" category.
- **[2026-07-15]**: Re-architected `scraper_livextv.py` to directly consume the DaddyLive JSON API (`api.vixnuvew.uk/api/channels`), replacing the hardcoded LiveXTV layout. (1) Fetches and caches the DaddyLive API every 30 minutes to dynamically support 88+ Live 24/7 channels. (2) Implemented a strict whitelist filter to inject 29 specific English sports and entertainment channels into the proxy, while merging them with the original 4 premium/4K LiveXTV feeds missing from the API. (3) Integrated high-quality network logos directly from the API into the `main.py` M3U generator and proxy image endpoints to natively support guide images in Jellyfin.
- **[2026-07-19]**: Completely refactored `scraper_livextv.py` to replace Playwright with blazing-fast direct HTTP extraction (~100ms vs ~10s). (1) Decoded XOR-obfuscated JavaScript from embed pages to extract signed m3u8 URLs directly, completely bypassing aggressive anti-bot redirections (`disable-devtool.js`) that were failing in Docker. (2) Made hardcoded channels domain-resilient by cross-referencing their IDs against the DaddyLive API to automatically self-heal and resolve new embed domains when old ones (e.g., `ritzembeds.pages.dev`) die. (3) Implemented automatic background signed URL refreshes for LiveXTV streams in `main.py` when chunklists expire, ensuring 24/7 uninterrupted playback.
